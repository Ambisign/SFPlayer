    ...
    public partial class userlogin : Form
    {
        ...
        private void Userlogin()
        {
            ...
            if (txtUsername.Text == "" || txtTokenNo.Text == "")
            {
                ...
                MessageBox.Show("Username and Token no cannot be blank");
                return;
            }
            if (txtDealerCode.Text.Trim() != "" && txtDealerCode.Text != "AlenkaMyclaud")
            {
                ...
                if (dsDefault.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        lblWait.Visible = false;
                        MessageBox.Show("The dealer code you have inserted is wrong.");
                        txtDealerCode.Focus();
                        return;
                    }
            ...
            if (dfId != 0)
            {
                ...
                {
                    //MessageBox.Show("You have reached the maximum of your tokens. Please contact your administrator.");
                    //txtUsername.Focus();
                    //return;
                }
            }
            ...
            {
                lblWait.Visible = false;
                MessageBox.Show("Your username or token is wrong.");
                return;
            }
            else if (ds.Tables[0].Rows[0]["PlayerType"].ToString() != "Desktop")
            {
                lblWait.Visible = false;
                MessageBox.Show("This token is not matching with this license type.");
                return;
            }
            //else if ((ds.Tables[0].Rows[0]["MusicType"].ToString() != "Copyright") && (ds.Tables[0].Rows[0]["MusicType"].ToString() != "NativeCR"))
            //{
            //    lblWait.Visible = false;
            //    MessageBox.Show("This token is not matching with this music type.");
            //    return;
            //}
            ...
            cmdLocal.CommandText = "insert into tbMisc(DealerCode,IsStore,DfClientId,IsAdvt,IsLock,PlayerVersion,ScheduleType) values('" + StaticClass.DealerCode + "'," + Convert.ToByte(StaticClass.IsStore) + "," + StaticClass.dfClientId + "," + Convert.ToByte(StaticClass.IsAdvt) + "," + Convert.ToByte(StaticClass.IsLock) + "," + StaticClass.PlayerVersion + ",'" + StaticClass.ScheduleType + "')";
        ...
        private void CheckClientsRights()
        {
            ...
            {
                MessageBox.Show("!! Purchase the subscription of music player  !!");
                Application.Exit();
                return;
            }
            if (ExpiryCopyrightStatus == "Yes" )
            {
                MessageBox.Show("!! The license of your music player is expired. Please contact your dealer. !!");
                Application.Exit();
                return;
            }
            ...
            if (ExpiryCopyrightStatus == "Yes")
            {
                MessageBox.Show("!! Subscription is Expired.Please connect your vendor !!");
                Application.Exit();
                return;
            }
