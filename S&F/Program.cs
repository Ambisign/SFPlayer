        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                if (processes.Length > 1)
                {
                    Application.Exit();
                    return;
                    // MessageBox.Show("Application is already running", "Alenka-Myclaud Player");
                ...
                if (File.Exists(filename))
                {
                    ...
                    if (dsOption.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            StaticClass.PlayerExpiryMessage = "Purchase the subscription of music player. Please contact our support team at " + StaticClass.MainwindowMessage;
                            Application.SetCompatibleTextRenderingDefault(false);
