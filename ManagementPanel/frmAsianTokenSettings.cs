...
{
    public partial class frmAsianTokenSettings : Form
    {
        gblClass objMainClass = new gblClass();
        DateTimePicker dtpOrder;
        public frmAsianTokenSettings()
        {
            InitializeComponent();
        ...
        private void frmCopyleftTokenSettings_Load(object sender, EventArgs e)
        {
            ...
            //{
            //    MessageBox.Show("Please check your Internet connection.", "Panel");
            //    return;
            //}
        ...
        private void cmbClientName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            sQr = sQr + " and PlayerType='Desktop'and  AMPlayerTokens.isAsian=1  order by Users.UserID desc  ";
        ...
        private void InitilizeAccountSettingsGrid()
        {
            ...
            Dam.HeaderText = "Asian";
            Dam.DataPropertyName = "Asian";
            dgAccountSettings.Columns.Add(Dam);
            Dam.Width = 50;
            Dam.Visible = true;
            dgAccountSettings.Columns[2].AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
            //3
            dgAccountSettings.Columns.Add("AsianExpiryDate", "Expiry Date");
            dgAccountSettings.Columns["AsianExpiryDate"].Width = 110;
            dgAccountSettings.Columns["AsianExpiryDate"].Visible = true;
            dgAccountSettings.Columns["AsianExpiryDate"].ReadOnly = false;
            dgAccountSettings.Columns["AsianExpiryDate"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
        ...
        private void FillTokenGenerationData(string sQr)
        {
            ...
            if (dtDetail.Rows.Count > 0)
            {
                for (int i = 0; i <= dtDetail.Rows.Count - 1; i++)
                {
                    ...
                    dgAccountSettings.Rows[dgAccountSettings.Rows.Count - 1].Cells[2].Value = dtDetail.Rows[i]["isAsian"];
                    dgAccountSettings.Rows[dgAccountSettings.Rows.Count - 1].Cells[3].Value = string.Format("{0:dd/MMM/yyyy}", dtDetail.Rows[i]["AsianExpiryDate"]);
        ...
        private void SaveUserToken()
        {
            for (int i = 0; i <= dgAccountSettings.Rows.Count - 1; i++)
            {
                ...
                cmd.Parameters["@StreamExpiryDate"].Value = dgAccountSettings.Rows[i].Cells[3].Value;
                ...
                cmd.Parameters["@IsAsian"].Value = Convert.ToInt32(dgAccountSettings.Rows[i].Cells[2].Value);

                cmd.Parameters.Add(new SqlParameter("@AsianExpiryDate", SqlDbType.DateTime));
                cmd.Parameters["@AsianExpiryDate"].Value = dgAccountSettings.Rows[i].Cells[3].Value;
