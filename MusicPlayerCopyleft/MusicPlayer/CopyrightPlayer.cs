    ...
    public partial class CopyrightPlayer : Form
    {
        ...
        private void PopulateInputFileTypeDetail(DataGridView dgGrid, Int32 currentPlayRow)
        {
            ...
            else
            {
                ...
                mlsSql = mlsSql + " where TitlesInPlaylists.PlaylistID=" + Convert.ToInt32(currentPlayRow) + "   ORDER BY   Rnd(-(date()*Titles.TitleID)*Time())";
            }
        ...
        private void mainwindow_Load(object sender, EventArgs e)
        {
            ...
            if (ObjMainClass.CheckForInternetConnection() == false)
            {
                ...
                {
                    MessageBox.Show("No local songs are found, please connect to Internet." );
                    Application.Exit();
                }
        ...
        private void btnFade_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //MessageBox.Show("Drag & drop the song in the player." );
                    return;
                }
                drawLine = false;
                dgPlaylist.Invalidate();
        ...
        private void panWmp2_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (SongFind == false)
            {
                if (IsCopyFromLocalList == "No")
                {
                    if (ObjMainClass.CheckForInternetConnection() == false)
                    {
                       // MessageBox.Show("Please Please check your Internet connection.." );
                        return;
                    }
                }
        ...
        private void SaveDefaultPlaylist(string DefaultSongId)
        {
            ...
            //    FillLocalPlaylist();
            //   // MessageBox.Show("This playlist name is already used." );
            //    return;
            //}
            //else if (StaticClass.Is_Admin != "1")
            //{
            //    MessageBox.Show(ObjMainClass.MainMessage );
            //    return;
            //}
            DefaultPlaylistSave("Music");
            FillLocalPlaylist();
        ...
        private void txtPlaylistName_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                       // MessageBox.Show("The playlist cannot be empty or without a name." );
                        return;
                    }
                    else if (pAction == "New")
                    {
        ...
        private void dgLocalPlaylist_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dgLocalPlaylist.Rows[dgLocalPlaylist.CurrentCell.RowIndex].Cells[2].Value.ToString() == "Default")
                    {
                        MessageBox.Show("It is not possible to delete the default playlist." );
                        return;
                    }
                    string sgr = "";
                    if (dgLocalPlaylist.Rows[dgLocalPlaylist.CurrentCell.RowIndex].Cells[2].Value.ToString() == "Default")
                        ...
                        if (musicPlayer1.URL != "")
                        {
                            ...
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                                for (int i = 0; i < dgPlaylist.Rows.Count; i++)
                                {
                                    if (dgPlaylist.Rows[i].Cells[0].Value.ToString() == MusicPlayer1CurrentSongId.ToString())
                                    {
                                        MessageBox.Show("It is not possible to delete the current playlist while playing." );
                                        return;
                                    }
                                }
                            }
        ...
        private void picSongRemove_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgLocalPlaylist.Rows[dgLocalPlaylist.CurrentCell.RowIndex].Cells[2].Value.ToString() == "Default")
                {
                    ...
                    {
                        MessageBox.Show("It is not possible to delete songs from the default playlist." );
                        return;
                    }
                    ...
                    if (musicPlayer1.URL != "")
                    {
                        ...
                        {
                            MessageBox.Show("It is not possible to delete the current song." );
                            return;
                        }
                    }
                    if (musicPlayer2.URL != "")
        ...
        private void SetDefault(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("To set for default playlist you need to add minimum 1 song in this playlist." );
                return;
            }
        ...
        private void btnSaveSettings_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MessageBox.Show("Manual Settings Save" );
        ...
        private void btnDefaultSettings_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MessageBox.Show("Default Settings Save" );
                StaticClass.IsAdvtManual = false;
        ...
        private void bgSaveDataWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i <= dgSaveDataGrid.Rows.Count - 1; i++)
            {
                Srno = Srno + 1;
                if (StaticClass.LocalCon.State == ConnectionState.Closed) { StaticClass.LocalCon.Open(); }
                sWr = "insert into TitlesInPlaylists(PlaylistID,TitleID,SrNo) values (" + Convert.ToInt32(SaveDataCurrentPlaylistId) + " , ";
        ...
        private void lblSongDurationOne_Click(object sender, EventArgs e)
        {
            
             lblPlayState.Text = "Player 1: " + musicPlayer1.playState.ToString() + Environment.NewLine + "Player 2: " + musicPlayer2.playState.ToString();
        }
        ...
        private void PlaySongNow(DataGridView dgGrid, string songIdWeb)
        {
            try
            {
                ...
                localfilename = Application.StartupPath + "\\so\\" + localSongId + ".mp3";
                TempSongName = Application.StartupPath + "\\so\\" + localSongId + ".sec";
                ...
                if (File.Exists(TempSongName))
                {
                    ...
                    {
                        DecrpetSec(Convert.ToInt32(localSongId));
        ...
        private void wcDownloadRequestSong_DownloadFileCompleted(object sender, AsyncCompletedEventArgs e)
        {
            ...
            {
                progressBar1.Value = 0;
                string str = "select title, (select name from artists where artistid=titles.artistid) as aName from titles where titleid= " + RequestSongId;
                DataTable dtRequest = new DataTable();
                dtRequest = ObjMainClass.fnFillDataTable(str);
                

                string TempSongName = Application.StartupPath + "\\so\\" + RequestSongId + ".mp3";
                long DownloadedSize = new FileInfo(TempSongName).Length;
                    ...
                    DisplaySongPlayerOne();
                        Song_Set_foucs();
                    lblSongName.Text = dtRequest.Rows[0]["title"].ToString();
                    lblArtistName.Text = dtRequest.Rows[0]["aName"].ToString();
                }
