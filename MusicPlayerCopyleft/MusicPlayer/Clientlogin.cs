...
namespace StoreAndForwardPlayer
{
    public partial class Clientlogin : Form
    {
        ...
        private void Clientlogin_Load(object sender, EventArgs e)
        {
            ...
            try {
                ...
                for (int iTitle = 0; iTitle < dtMisTitle.Rows.Count; iTitle++)
                {
                    ...
                    if (StaticClass.LocalCon.State == ConnectionState.Closed) { StaticClass.LocalCon.Open(); }
                    string sWr = "insert into Titles values (" + Convert.ToInt32(dtMisTitle.Rows[iTitle]["TitleID"]) + " , " + Convert.ToInt32(dtMisTitle.Rows[iTitle]["AlbumID"]) + " , ";
            ...
            cmdLocal.Connection = StaticClass.LocalCon;
            cmdLocal.CommandText = "insert into tbMisc(DealerCode,IsStore,DfClientId,IsAdvt,IsLock,PlayerVersion,support,ScheduleType) values('" + StaticClass.DealerCode + "'," + Convert.ToByte(StaticClass.IsStore) + "," + StaticClass.dfClientId + "," + Convert.ToByte(StaticClass.IsAdvt) + "," + Convert.ToByte(StaticClass.IsLock) + "," + StaticClass.PlayerVersion + " ,'" + StaticClass.MainwindowMessage + "','" + StaticClass.ScheduleType + "')";
            cmdLocal.ExecuteNonQuery();
            StaticClass.LocalCon.Close();
            //if (ObjMainClass.CheckForInternetConnection() == true)
            //{
        ...
        private void SubmitValidation()
        {
            ...
            if (txtloginUserName.Text == "")
            {
                MessageBox.Show("Login user name cannot be blank");
                SubmitValidate = "False";
            }
            else if (txtLoginPassword.Text == "")
            {
                MessageBox.Show("Login password cannot be blank");
                SubmitValidate = "False";
            }
            else if (ds.Tables[0].Rows.Count <= 0)
            {
                MessageBox.Show("Login user/password is wrong");
                SubmitValidate = "False";
            }
            else if (ds.Tables[0].Rows.Count > 0)
            {
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            if (ObjMainClass.CheckForInternetConnection() == true)
            {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("Local are songs not found.Please connect your internet to download the songs");
                        return;
                    }
